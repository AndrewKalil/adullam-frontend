...
import type { Discount, DiscountInsert, DiscountUpdate } from "./discounts.types";
...
export const updateDiscount = (options: { id: string; payload: DiscountUpdate }): Promise<Discount> => {
  const { id, payload } = options;
  return apiClient.patch<Discount>(generatePath(ApiEndpoint.DiscountsById, { id }), payload);
};
